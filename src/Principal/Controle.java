...
public class Controle {	
	...
	public static String carregar(String caminho) {
		...
		String linha = "", texto = "";//Cria as variáveis que vão receber texto.
		try {
			arquivo = new FileReader(caminho);//Abre um arquivo.
			leitor = new BufferedReader(arquivo);//Criando leitor de arquivo.
			while(linha != null) {//Copia uma linha até a linha ser nula(fim).
				linha = leitor.readLine();//Copia uma linha do arquivo para variável.
				if(linha != null) {
					texto += linha + "\n";
				}				
			}
	...
	public static void msgAtividade(Instant tempoInicial) {
		...
		if((duracao.toMillis()%intervalo) <= 2) {
			System.out.println("...AG em execu��o..." + duracao);
		}		
	}
